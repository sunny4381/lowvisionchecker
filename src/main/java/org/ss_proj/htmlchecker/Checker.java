...
import org.eclipse.actf.visualization.eval.*;
...
public class Checker {
    ...
    public List<IProblemItem> getProblemList() {
        return checkResult.getProblemList();
    }
...
//            }
    ...
    private boolean hasFrameset(Document document, IWebBrowserACTF webBrowser) throws MalformedURLException, FileNotFoundException, UnsupportedEncodingException {
        ...
        if (framesetNl.getLength() > 0) {
            ...
            try (PrintWriter fileOutput = new PrintWriter(new OutputStreamWriter(new FileOutputStream(sFileName), "UTF-8"))) { //$NON-NLS-1$
