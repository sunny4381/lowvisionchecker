...
public abstract class NLS {
	...
	public static void initializeMessages(final String baseName, final Class<?> clazz) {
		initializeMessages(baseName, clazz, Charset.defaultCharset());
	}
	 ...
	 * @param baseName the base name of a fully qualified message properties file.
	 * @param clazz the class where the constants will exist
	 * @param chaset the chaset of resource bundles.
	 */
	public static void initializeMessages(final String baseName, final Class<?> clazz, Charset chaset) {
		if (System.getSecurityManager() == null) {
			load(baseName, clazz, chaset);
			return;
		}
	...
	static void load(final String bundleName, Class<?> clazz, Charset charset) {
		...
		for (String variant : variants) {
			...
			final InputStream is = loader == null ? ClassLoader.getSystemResourceAsStream(variant) : loader.getResourceAsStream(variant);
			if (is == null)
				continue;
			try (final InputStreamReader input = new InputStreamReader(is, charset)) {
